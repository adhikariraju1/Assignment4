...
Simple regression plot: *Raju describe what the plot it shows*
...
Stargazer of the Pooled OLS
```{r regressionpooled, results='asis'}
stargazer::stargazer(m4, type='latex', digits = 2, 
          title = 'Effect of Income Inequality on EPI', font.size = 'normalsize')
```

Residual vs Fitted Plot: *Raju interpret it and make case on why we should consider FE or RE estimates*
...
Use stargazer output of FE model and check the F-statistic. 
```{r regressionfixed, results='asis'}
stargazer::stargazer(m5, type='latex', digits = 2, 
          title = 'Effect of Income Inequality on EPI', font.size = 'normalsize')
```
...
phtest(m5, m6)  
```
Then show all the regressions in one place and interpret the most effective estimation method.
